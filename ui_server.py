...
# Dynamic Configuration Paths (Resolves safely on both Bare-Metal and Docker)
...
# Uvicorn Custom Access Log Filter to completely silence healthcheck logging
...
def load_clean_json(filepath):
    """Safely loads JSON by stripping out inline C-style and Hash comments first."""
...
def load_vault_safe(filepath):
    """Loads the vault and migrates flat legacy structures into the new App/User format."""
        ...
        <form hx-post="/save/config" hx-target="#status" @submit="document.getElementById('config_payload').value = preparePayload()">
            ...
            <div x-show="tab === 'pushover'">
                <div class="card">
                        ...
                        <div style="flex: 0 0 auto;">
                            <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;">
                                ...
                                <label class="clickable-label">
                                    <input type="checkbox" x-model="pushGlobals._isTokenAlias" style="width: auto; margin: 0;"> (Use Alias)
                                </label>
                            </div>
                            <template x-if="!pushGlobals._isTokenAlias">
                                <input class="token-input" x-model="pushGlobals.token" maxlength="30" placeholder="Required for Catch-All" autocomplete="new-password" data-lpignore="true" data-bwignore="true" data-1p-ignore="true">
                            </template>
                            <template x-if="pushGlobals._isTokenAlias">
                                <select class="auto-width" x-model="pushGlobals.token">
                                    <option value="">-- Select App Alias --</option>
                                    <template x-for="alias in vaultAppAliases" :key="alias">
                                        <option :value="alias" x-text="alias" :selected="pushGlobals.token === alias"></option>
                                    </template>
                                </select>
                            </template>
                        </div>
                    ...
                    <div class="flex-row">
                        <div style="flex: 0 0 auto;">
                            <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;">
                                ...
                                <label class="clickable-label">
                                    <input type="checkbox" x-model="pushGlobals._isUserAlias" style="width: auto; margin: 0;"> (Use Alias)
                                </label>
                            </div>
                            <template x-if="!pushGlobals._isUserAlias">
                                <input class="token-input" x-model="pushGlobals.user" maxlength="30" placeholder="Required for Catch-All" autocomplete="new-password" data-lpignore="true" data-bwignore="true" data-1p-ignore="true">
                            </template>
                            <template x-if="pushGlobals._isUserAlias">
                                <select class="auto-width" x-model="pushGlobals.user">
                                    <option value="">-- Select User Alias --</option>
                                    <template x-for="alias in vaultUserAliases" :key="alias">
                                        <option :value="alias" x-text="alias" :selected="pushGlobals.user === alias"></option>
                                    </template>
                                </select>
                            </template>
                        </div>
                    ...
                    <button type="button" class="outline" @click="showGlobalAdv = !showGlobalAdv" style="margin-bottom: 1rem; margin-top: 0.5rem;">
                        <span x-text="showGlobalAdv ? 'Hide Advanced Globals' : 'Show Advanced Globals'"></span>
                    </button>
                    ...
                    <div x-show="showGlobalAdv" class="adv-card">
                        <div class="flex-row" style="margin-bottom: 1rem;">
                            ...
                            <div class="flex-col" style="flex: 1; min-width: 200px;">
                                ...
                                <input x-model="pushGlobals.device" placeholder="All Devices" style="width: 100%;">
                            </div>
                        </div>
                        <div class="flex-row" x-show="pushGlobals.priority == 2" style="margin-bottom: 1rem;">
                            <div style="flex: 0 0 auto;">
                                <label>Retry (sec) &gt;= 30</label>
                                <input type="number" x-model.number="pushGlobals.retry" min="30" style="width: 120px;">
                            </div>
                            <div style="flex: 0 0 auto;">
                                <label>Expire (sec) &lt;= 10800</label>
                                <input type="number" x-model.number="pushGlobals.expire" max="10800" style="width: 120px;">
                            </div>
                        </div>
                        <div class="flex-row" style="margin-bottom: 1rem;">
                            <div class="flex-col" style="flex: 1; min-width: 200px;">
                                <label>Supplementary URL</label>
                                <input x-model="pushGlobals.url" placeholder="Optional" maxlength="512" style="width: 100%;">
                            </div>
                        </div>
                        <div class="flex-row" style="margin-bottom: 1rem;">
                            <div style="flex: 0 0 auto;">
                                <label>URL Title</label>
                                <input x-model="pushGlobals.url_title" placeholder="Optional" maxlength="100" style="width: 100ch; max-width: 100%;">
                            </div>
                        </div>
                    </div>

                    <div class="flex-row" style="gap: 1.5rem; margin-top: 0.5rem;">
                        <label class="clickable-label" style="font-weight: normal;">
                            <input type="checkbox" x-model="pushGlobals.disable_attachments" style="width: auto; margin: 0;"> Disable Image Attachments
                        </label>
                        <label class="clickable-label" style="font-weight: normal;">
                            <input type="checkbox" x-model="pushGlobals.force_plaintext" style="width: auto; margin: 0;"> Force Plaintext Formatting
                        </label>
                        <label class="clickable-label" style="font-weight: normal;">
                            <input type="checkbox" x-model="pushGlobals.disable_persistence" style="width: auto; margin: 0;"> Disable Disk Persistence
                        </label>
                    </div>
                </div>
                        ...
                        <div class="card" style="background: var(--bg-color);">
                            <div class="flex-row" style="align-items: flex-start;">
                                <div style="flex: 0 0 auto;">
                                    ...
                                    <select class="auto-width" x-model="map.match">
                                ...
                                <div style="flex: 1; min-width: 200px;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;">
                                        ...
                                        <label class="clickable-label" style="margin: 0;">
                                            <input type="checkbox" x-model="map._isRegex" style="width: auto; margin: 0;"> (Use Regex)
                                        </label>
                                    </div>
                                ...
                                <div style="flex: 0 0 auto;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;">
                                        ...
                                        <label class="clickable-label" style="margin: 0;">
                                            <input type="checkbox" x-model="map._isTokenAlias" style="width: auto; margin: 0;"> (Use Alias)
                                        </label>
                                    </div>
                                    <template x-if="!map._isTokenAlias">
                                        <input class="token-input" x-model="map.token" maxlength="30" placeholder="Required Override" autocomplete="new-password" data-lpignore="true" data-bwignore="true" data-1p-ignore="true">
                                    </template>
                                    <template x-if="map._isTokenAlias">
                                        <select class="auto-width" x-model="map.token">
                                            <option value="">-- Select App Alias --</option>
                                            <template x-for="alias in vaultAppAliases" :key="alias">
                                                <option :value="alias" x-text="alias" :selected="map.token === alias"></option>
                                            </template>
                                        </select>
                                    </template>
                                </div>
                            ...
                            <button type="button" class="outline" @click="map._showAdv = !map._showAdv" style="margin-bottom: 1rem; font-size: 0.8rem; padding: 0.3rem 0.5rem;">
                                <span x-text="map._showAdv ? 'Hide Route Settings' : 'Show Route Settings'"></span>
                            </button>
                            ...
                            <div x-show="map._showAdv" class="adv-card">
                                <div class="flex-row" style="margin-bottom: 1.5rem; justify-content: flex-start;">
                                    <div style="flex: 0 0 auto;">
                                        <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;">
                                            ...
                                            <label class="clickable-label" style="margin: 0;">
                                                <input type="checkbox" x-model="map._isUserAlias" style="width: auto; margin: 0;"> (Use Alias)
                                            </label>
                                        </div>
                                        <template x-if="!map._isUserAlias">
                                            <input class="token-input" x-model="map.user" maxlength="30" placeholder="Inherit Global" autocomplete="new-password" data-lpignore="true" data-bwignore="true" data-1p-ignore="true">
                                        </template>
                                        <template x-if="map._isUserAlias">
                                            <select class="auto-width" x-model="map.user">
                                                <option value="">-- Inherit Global --</option>
                                                <template x-for="alias in vaultUserAliases" :key="alias">
                                                    <option :value="alias" x-text="alias" :selected="map.user === alias"></option>
                                                </template>
                                            </select>
                                        </template>
                                    </div>
                                    <div style="flex: 0 0 auto;">
                                        ...
                                        <select class="auto-width" x-model="map.priority">
                                            <option value="">Inherit Global</option>
                                            <option value="-2">Lowest (-2)</option>
                                            <option value="-1">Low (-1)</option>
                                            <option value="0">Normal (0)</option>
                                    ...
                                    <div class="flex-col" style="flex: 1; min-width: 150px;">
                                        <label>Target Device</label>
                                        <input x-model="map.device" placeholder="Inherit Global" style="width: 100%;">
                                    </div>
                                </div>

                                <div class="flex-row" x-show="map.priority == 2" style="margin-bottom: 1rem;">
                                    <div style="flex: 0 0 auto;">
                                        <label>Retry (sec)</label>
                                        <input type="number" x-model.number="map.retry" min="30" style="width: 120px;">
                                    </div>
                                    <div style="flex: 0 0 auto;">
                                        <label>Expire (sec)</label>
                                        <input type="number" x-model.number="map.expire" max="10800" style="width: 120px;">
                                    </div>
                                </div>
                                ...
                                <div class="flex-row" style="margin-bottom: 0.5rem;">
                                    <div class="flex-col" style="flex: 1; min-width: 200px;">
                                        <label>Supplementary URL</label>
                                        <input x-model="map.url" placeholder="Inherit Global" maxlength="512" style="width: 100%;">
                                    </div>
                                </div>
                                ...
                                <div class="flex-row" style="margin-bottom: 1rem;">
                                    <div style="flex: 0 0 auto;">
                                        <label>URL Title</label>
                                        <input x-model="map.url_title" placeholder="Inherit Global" maxlength="100" style="width: 100ch; max-width: 100%;">
                                    </div>
                                </div>

                                <div class="flex-row" style="gap: 1.5rem; margin-top: 1rem;">
                                    <label class="clickable-label" style="font-weight: normal;">
                                        <input type="checkbox" x-model="map.disable_attachments" style="width: auto; margin: 0;"> Disable Image Attachments
                                    </label>
                                    <label class="clickable-label" style="font-weight: normal;">
                                        <input type="checkbox" x-model="map.force_plaintext" style="width: auto; margin: 0;"> Force Plaintext Formatting
                                    </label>
                                    <label class="clickable-label" style="font-weight: normal;">
                                        <input type="checkbox" x-model="map.disable_persistence" style="width: auto; margin: 0;"> Disable Disk Persistence
                                    </label>
                                </div>
                            </div>

                            <div style="text-align: right;">
                                <button type="button" class="danger" @click="mappings.splice(idx, 1)">Remove Mapping</button>
                            </div>
                        </div>
                    </template>
            ...
            <div x-show="tab === 'server'">
                <div class="card">
                    ...
                    <div class="flex-row" style="margin-bottom: 1rem;">
                        <div style="flex: 0 0 auto;">
                            ...
                            <select class="auto-width" x-model="smtp.loglevel">
                    ...
                    <div class="flex-row" style="margin-bottom: 1rem;">
                        <div class="flex-col" style="flex: 1; min-width: 200px;">
                            <label>Disk Queue Directory</label>
                            <input x-model="smtp.queue_dir" placeholder="/tmp/queue" style="width: 100%;">
                        </div>
                    </div>
                    <div class="flex-row" style="margin-bottom: 1rem;">
                        <div class="flex-col" style="flex: 1; min-width: 200px;">
                            <label>Global Hostname (Greeting Banner)</label>
                            <input x-model="smtp.hostname" placeholder="gateway.local" style="width: 100%;">
                        </div>
                    </div>
                    <div class="flex-row">
                        <div class="flex-col" style="flex: 1; min-width: 200px;">
                            <label>Global TLS Cert File (Fallback)</label>
                            <input x-model="smtp.tls_cert_file" placeholder="/etc/ssl/certs/global.pem" style="width: 100%;">
                        </div>
                        <div class="flex-col" style="flex: 1; min-width: 200px;">
                            <label>Global TLS Key File (Fallback)</label>
                            <input x-model="smtp.tls_key_file" placeholder="/etc/ssl/private/global.key" style="width: 100%;">
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="adv-card" style="margin-bottom: 1.5rem; border-left-color: var(--primary-color);">
                        ...
                        <div class="flex-row">
                            <div class="flex-col">
                                <input type="text" x-model="newSmtpUser" placeholder="Username" autocomplete="new-password" data-lpignore="true" data-bwignore="true" data-1p-ignore="true">
                            </div>
                            <div class="flex-col">
                                <input type="password" x-model="newSmtpPass" placeholder="Password" autocomplete="new-password" data-lpignore="true" data-bwignore="true" data-1p-ignore="true">
                            </div>
                            <div style="flex: 0 0 auto; display: flex; gap: 0.5rem;">
                                <button type="button" @click="addSmtpUser()">Add User</button>
                                <button type="button" class="outline" @click="newSmtpUser = ''; newSmtpPass = '';">Clear</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th class="table-col-min">Last Modification</th>
                                ...
                                <tr>
                                    ...
                                    <td class="table-col-min">
                                        <span class="time-display"
                                              x-text="formatTime(smtp_meta[name])"
                                              :title="getFullTime(smtp_meta[name])">
                                        </span>
                                    </td>
                                    <td class="table-col-min">
                                        <button type="button" class="outline" @click="openEditModal('smtp', name)">Change Password</button>
                                        <button type="button" class="danger" @click="delete smtp.auth[name]; delete smtp_meta[name];">Remove</button>
                                    </td>
                                </tr>
                            </template>
                        </tbody>
                        ...
                        <div class="card" style="background: var(--bg-color);">
                            <div class="flex-row" style="align-items: flex-end; margin-bottom: 1rem;">
                                <div style="flex: 0 0 auto;">
                                    ...
                                    <input x-model="l.bind" placeholder="0.0.0.0:25" maxlength="52" style="width: 55ch; max-width: 100%;">
                                </div>
                                <div style="flex: 0 0 auto; padding-bottom: 0.5rem;">
                                    <label class="clickable-label" style="font-weight: normal; white-space: nowrap;">
                                        <input type="checkbox" x-model="l.starttls" style="margin: 0;"> STARTTLS
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="flex-row" style="margin-bottom: 1rem;">
                                <div class="flex-col" style="flex: 1; min-width: 200px;">
                                    ...
                                    <input x-model="l.hostname" placeholder="Optional" style="width: 100%;">
                                </div>
                            </div>

                            <div class="flex-row" x-show="l.starttls" style="margin-bottom: 1rem;">
                                <div class="flex-col">
                                    <label>Specific TLS Cert File</label>
                                    <input x-model="l.tls_cert_file" placeholder="Leaves blank to use Global">
                                </div>
                                <div class="flex-col">
                                    <label>Specific TLS Key File</label>
                                    <input x-model="l.tls_key_file" placeholder="Leaves blank to use Global">
                                </div>
                            </div>
                            <div style="text-align: right;">
                                <button type="button" class="danger" @click="smtp.listeners.splice(idx, 1)">Remove Listener</button>
                            </div>
                        </div>
                    </template>
        ...
        <div x-show="tab === 'vault'">
            <div class="card">
                ...
                <form @submit.prevent="addVaultToken()">
                    <div class="flex-row" style="gap: 1rem; align-items: flex-end; justify-content: flex-start;">
                        <div style="flex: 0 0 auto;">
                            ...
                            <select class="auto-width" x-model="newVaultType">
                        ...
                        <div style="flex: 1; min-width: 150px;">
                            ...
                            <input type="text" x-model="newVaultName" required autocomplete="off" data-lpignore="true" data-bwignore="true" data-1p-ignore="true" style="width: 100%;">
                        </div>
                        <div style="flex: 0 0 auto;">
                            <label>Token Value</label>
                            <input type="password" class="token-input" x-model="newVaultToken" maxlength="30" required autocomplete="new-password" data-lpignore="true" data-bwignore="true" data-1p-ignore="true">
                        </div>
                        <div style="flex: 0 0 auto; display: flex; gap: 0.5rem;">
                            <button type="submit">Add Token</button>
                            <button type="button" class="outline" @click="newVaultName=''; newVaultToken='';">Clear</button>
                        </div>
                    </div>
                </form>
                ...
                <table style="margin-bottom: 2rem;">
                    <thead>
                        <tr>
                            <th style="width: auto;">Alias Name</th>
                            <th class="table-col-min">Last Modification</th>
                            <th class="table-col-min">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        <template x-for="(v, idx) in vaultApp" :key="v.name">
                            <tr>
                                ...
                                <td class="table-col-min">
                                    <span class="time-display"
                                          x-text="formatTime(v.epoch)"
                                          :title="getFullTime(v.epoch)">
                                    </span>
                                </td>
                                <td class="table-col-min">
                                    <button type="button" class="outline" @click="openEditModal('vault', v.name, 'app', idx)">Modify Secret</button>
                                    <button type="button" class="danger" @click="deleteVaultToken('app', idx)">Delete</button>
                                </td>
                            </tr>
                        </template>
                    </tbody>
                    ...
                    <tbody>
                        <template x-for="(v, idx) in vaultUser" :key="v.name">
                            <tr>
                                ...
                                <td class="table-col-min">
                                    <button type="button" class="outline" @click="openEditModal('vault', v.name, 'user', idx)">Modify Secret</button>
                                    <button type="button" class="danger" @click="deleteVaultToken('user', idx)">Delete</button>
                                </td>
                            </tr>
                        </template>
                    </tbody>
            ...
            <form hx-post="/save/ui" hx-target="#status">
                <div class="card">
                    ...
                    <div class="form-group" style="margin-bottom: 1rem;">
                        ...
                        <input type="number" name="port" value="{{ ui_port }}" required style="width: 120px; max-width: 100%;" oninput="if(this.value.length > 5) this.value = this.value.slice(0, 5);" min="1" max="65535">
                    </div>
                    <div class="flex-row">
                        <div style="flex: 0 0 auto;">
                        ...
                        <div style="flex: 0 0 auto;">
                            ...
                            <select class="auto-width" name="date_format" x-model="ui_fmt">
                                <option value="YYYY-MM-DD HH:mm:ss">ISO (YYYY-MM-DD HH:mm:ss)</option>
                                <option value="MM/DD/YYYY hh:mm:ss A">US (MM/DD/YYYY hh:mm:ss A)</option>
                                <option value="DD/MM/YYYY HH:mm:ss">UK/EU (DD/MM/YYYY HH:mm:ss)</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom: 0.5rem; margin-top: 1rem;">
                        <label class="clickable-label" style="font-weight: normal;">
                            <input type="checkbox" name="relative_time" x-model="ui_relative" style="width: auto; margin: 0;"> Render Coerced Relative Human Times (e.g. '3 days ago')
                        </label>
                    </div>
                    <div style="margin-bottom: 1.5rem;">
                        <label class="clickable-label" style="font-weight: normal;">
                            <input type="checkbox" name="expand_adv" x-model="ui_expand_adv" style="width: auto; margin: 0;"> Always Expand Route Settings by Default
                        </label>
                    </div>
                    ...
                    <div style="margin-bottom: 1rem;">
                        <label class="clickable-label" style="font-weight: normal;">
                            <input type="checkbox" name="https" x-model="httpsEnabled" style="width: auto; margin: 0;"> Enable HTTPS Bindings
                        </label>
                    </div>

                    <div class="flex-row" x-show="httpsEnabled">
                        <div class="flex-col">
                            <label>UI Specific TLS Cert File</label>
                            <input type="text" name="tls_cert" value="{{ ui_cert }}" placeholder="Leave blank for auto-generated UUID cert">
                        </div>
                        <div class="flex-col">
                            <label>UI Specific TLS Key File</label>
                            <input type="text" name="tls_key" value="{{ ui_key }}">
                        </div>
                    </div>
                </div>
        ...
        <div x-show="editModal.open" class="modal-overlay" style="display: none;" x-transition>
            <div class="modal-content" @click.away="editModal.open = false">
                ...
                <div style="display: flex; gap: 1rem; justify-content: flex-end; margin-top: 1.5rem;">
                    <button type="button" class="outline" @click="editModal.open = false">Cancel</button>
                    <button type="button" @click="saveEditModal()">Save Changes</button>
                </div>
            </div>
        </div>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('gatewaySettings', () => ({
            ...
            init() {
                ...
                // Inflate Vault Structures
                for(const [k, v] of Object.entries(this.vaultMeta.app || {})) {
                    this.vaultApp.push({ name: k, epoch: v, token: '__RETAIN__' });
                    this.vaultAppAliases.push(k);
                }
                for(const [k, v] of Object.entries(this.vaultMeta.user || {})) {
                    this.vaultUser.push({ name: k, epoch: v, token: '__RETAIN__' });
                    this.vaultUserAliases.push(k);
                }

                // Inflate Configurations
                ...
                for(const [k, v] of Object.entries(po)) {
                    if(gKeys.includes(k)) {
                        ...
                        let isRegex = false;
                        let displayKey = k;
                        if (k.toLowerCase().startsWith('regex:')) {
                            isRegex = true;
                            displayKey = k.substring(6);
                        }
            ...
            addMapping() {
                this.mappings.push({ _key: '', match: 'to', token: '', _isRegex: false, _isTokenAlias: false, _isUserAlias: false, _showAdv: this.ui_expand_adv, disable_attachments: false, force_plaintext: false, disable_persistence: false });
            },
            addListener() {
                this.smtp.listeners.push({ bind: '0.0.0.0:25', starttls: false, tls_cert_file: '', tls_key_file: '' });
            },
            addSmtpUser() {
                if(!this.newSmtpUser || !this.newSmtpPass) return;
                this.smtp.auth[this.newSmtpUser] = "RAW:" + this.newSmtpPass;
                this.smtp_meta[this.newSmtpUser] = Math.floor(Date.now() / 1000);
                this.newSmtpUser = '';
                this.newSmtpPass = '';
            },
            addVaultToken() {
                ...
                const existingIdx = target.findIndex(x => x.name === this.newVaultName);
                if(existingIdx >= 0) {
                    target[existingIdx].token = this.newVaultToken;
                    target[existingIdx].epoch = Math.floor(Date.now() / 1000);
                } else {
                    target.push({name: this.newVaultName, token: this.newVaultToken, epoch: Math.floor(Date.now() / 1000)});
                }

                // Immediately update routing dropdown array states to match new Vault additions
            ...
            deleteVaultToken(type, idx) {
                ...
                const aliasName = targetArr[idx].name;

                // Active Mapping Constraint Validation
                let inUse = false;
                if(type === 'app') {
                    if(this.pushGlobals._isTokenAlias && this.pushGlobals.token === aliasName) inUse = true;
                    this.mappings.forEach(m => { if(m._isTokenAlias && m.token === aliasName) inUse = true; });
                } else {
                    if(this.pushGlobals._isUserAlias && this.pushGlobals.user === aliasName) inUse = true;
                    this.mappings.forEach(m => { if(m._isUserAlias && m.user === aliasName) inUse = true; });
                }

                if(inUse) {
                    alert(`Error: Alias '${aliasName}' is actively assigned to an email route. Reconfigure your Pushover Rules before deleting.`);
                    return;
                }
            ...
            openEditModal(type, name, subType='', idx=null) {
            ...
            saveEditModal() {
                ...
                if(this.editModal.type === 'smtp') {
                    this.smtp.auth[this.editModal.name] = "RAW:" + this.editModal.value;
                    this.smtp_meta[this.editModal.name] = Math.floor(Date.now() / 1000);
                } else if(this.editModal.type === 'vault') {
                    const target = this.editModal.subType === 'app' ? this.vaultApp : this.vaultUser;
                    target[this.editModal.idx].token = this.editModal.value;
                    target[this.editModal.idx].epoch = Math.floor(Date.now() / 1000);
                }
                this.editModal.open = false;
            },
            preparePayload() {
                const finalPushover = { ...this.pushGlobals };
                finalPushover.attachments = !this.pushGlobals.disable_attachments;
                delete finalPushover.disable_attachments;
                delete finalPushover._isTokenAlias;
                delete finalPushover._isUserAlias;

                ['priority', 'retry', 'expire', 'ttl'].forEach(p => {
                    if (finalPushover[p] === '' || finalPushover[p] === null || finalPushover[p] === undefined) delete finalPushover[p];
                    else finalPushover[p] = parseInt(finalPushover[p], 10);
                });
                ['device', 'url', 'url_title', 'sound', 'tags', 'user'].forEach(p => {
                    if (finalPushover[p] === '') delete finalPushover[p];
                });

                this.mappings.forEach(m => {
                    if(m._key && m._key.trim() !== '') {
                        let k = m._key.trim();
                        if (m._isRegex && !k.toLowerCase().startsWith('regex:')) {
                            k = 'regex:' + k;
                        }

                        const { _key, _showAdv, _isTokenAlias, _isUserAlias, _isRegex, disable_attachments, ...rest } = m;
                        rest.attachments = !disable_attachments;
                        ['priority', 'retry', 'expire', 'ttl'].forEach(p => {
                            if (rest[p] === '' || rest[p] === null || rest[p] === undefined) delete rest[p];
                            else rest[p] = parseInt(rest[p], 10);
                        });
                        ['device', 'url', 'url_title', 'sound', 'tags', 'user'].forEach(p => {
                            if (rest[p] === '') delete rest[p];
                        });
                        finalPushover[k] = rest;
                    }
                });
                return JSON.stringify({ pushover: finalPushover, smtp: this.smtp, _smtp_meta: this.smtp_meta });
            },
            prepareVaultPayload() {
                return JSON.stringify({ app: this.vaultApp, user: this.vaultUser });
            }
        }));
    });
    </script>
...
def signal_smtp_app(restart_listeners=False):
    """Sends strict OS signals over shared PID namespace to the worker."""
    if not os.path.exists(SMTP_PID_FILE): return False
    with open(SMTP_PID_FILE, 'r') as f:
        pid = int(f.read().strip())
    try:
        os.kill(pid, signal.SIGUSR2) # Reload dynamic mappings
        if restart_listeners:
            time.sleep(5.0) # 5-second offset allows daemon complete file-read overhead
            os.kill(pid, signal.SIGUSR1) # Trigger port binding evaluation
        return True
    except ProcessLookupError: return False

@app.get("/healthcheck", response_class=PlainTextResponse)
async def healthcheck():
    """Unlogged endpoint to verify UI service health."""
    return "OK"
...
async def index():
    ...
    # Silent Plaintext Password Auto-Hashing Logic
...
async def save_vault_state(vault_json: str = Form(...)):
    try:
        ...
        # Hydrate active definitions into strict files. Handle partial replacements seamlessly.
        for vtype in ["app", "user"]:
            for item in parsed.get(vtype, []):
                ...
                if tok == "__RETAIN__":
                    new_data[vtype][name] = vault_data[vtype].get(name, "")
                else:
                    new_data[vtype][name] = tok
...
if __name__ == "__main__":
    ui_restart_event = threading.Event()
    ui_shutdown_event = threading.Event()

    signal.signal(signal.SIGUSR1, lambda signum, frame: ui_restart_event.set())
    signal.signal(signal.SIGINT, lambda signum, frame: ui_shutdown_event.set())
    signal.signal(signal.SIGTERM, lambda signum, frame: ui_shutdown_event.set())
    ...
    while not ui_shutdown_event.is_set():
        ...
        while t.is_alive():
            if ui_restart_event.is_set() or ui_shutdown_event.is_set():
                server.should_exit = True; ui_restart_event.clear()
                break
            time.sleep(1)
        t.join()
